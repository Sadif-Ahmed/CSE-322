...
NS_LOG_COMPONENT_DEFINE("Mobile High Speed Wifi");
...
class mobile_wifi
{
    ...
    uint32_t speed;
    uint32_t connector_datarate;
    uint32_t wifi_datarate;
    mobile_wifi(uint32_t nWifi,uint32_t flow,uint32_t packets_per_second,uint32_t packet_size,uint32_t speed)
    {
        ...
        this->speed=speed;
        this->wifi_datarate = packets_per_second*packet_size*8;
        this->connector_datarate = this->wifi_datarate/2;
    }
    ...
    //pointToPoint.SetDeviceAttribute("DataRate", StringValue(std::to_string(connector_datarate)));
    ...
    //  Ptr<MobilityModel> mob = sender_wifiStaNodes.Get(1)->GetObject<MobilityModel>();
    ...
    sender_helper.SetAttribute("DataRate", DataRateValue(DataRate(wifi_datarate)));
...
main(int argc, char* argv[])
{
    ...
    {
        std::cout << speed << "\t" << throughput  << std::endl;
    }
    else if(param == 8)
    {
        std::cout << speed << "\t" << packet_delivery_ratio  << std::endl;
    }
